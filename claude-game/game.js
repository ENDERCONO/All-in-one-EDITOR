/* =====================================================================
   ...
   New features:
   - 3 character choices: Pumpkin (default), Zaid, Rich — each with sprite sheets
   - Permanent ability upgrades: faster bullets, faster fire rate, shotgun spread,
     explosive bullets, ricochet, bigger bullets, triple shot, speed burst, etc.
   - 3 ability cards offered on level-up (was 2)
   - Leaderboard panel (top-right) with all players, level, and points
   - More small cover obstacles scattered around the map
   - SFX for shooting, dash, hit, death, level-up, shield
   ===================================================================== */
(function () {
  'use strict';
  ...
  const CHARACTERS = {
    pumpkin: {
      ...
      color: '#ff8c42',
      // map anim states to sprite filenames
      sprites: {
  ...
  const me = {
    ...
    mods:{
      ...
      bulletSpeed:0,    // +% bullet travel speed
      explosive:0,      // radius of explosion on impact (0=none)
      ricochet:0,       // number of bounces
      bigBullet:0,      // +radius multiplier
      spreadShot:0,     // shotgun pellet count bonus
      rapidBurst:0,     // burst count
    },
    abilities:[],
    points:0,           // total score (damage + kills)
  };
  let others={};
  const bullets=[];
  const particles=[];  // explosion / hit particles
  ...
  function buildObstacles(){
    ...
    // Small barrels / crates for close-range cover
    const smallRng=mulberry32(9999);
    for(let i=0;i<300;i++){
      const s=18+smallRng()*28; // small: 18–46px
      const x=100+smallRng()*(WORLD_W-200), y=100+smallRng()*(WORLD_H-200);
      // skip very center spawn
  ...
  function spawnParticles(x,y,color,count,speed,life){
    for(let i=0;i<count;i++){
      ...
      particles.push({ x,y, vx:Math.cos(a)*spd, vy:Math.sin(a)*spd,
        r:2+Math.random()*4, life, maxLife:life, color });
    }
  }
  function spawnExplosion(x,y,radius,dmg){
    ...
    // check others (register outgoing damage)
    ...
    for(const id in s){ if(id===myId) continue; const p=s[id];
      ...
      (p.shots||[]).forEach(sh=>{ if(seenShots.has(sh.id)) return; seenShots.add(sh.id);
        bullets.push({ id:sh.id, owner:id, x:sh.x, y:sh.y,
          ...
          explosive:sh.explosive||0, ricochet:sh.ricochet||0,
          radius:sh.radius||BULLET_R, born:t }); });
  ...
  function spawnBullet(angle, spd, dmg, ref, pierce, opts){
    ...
    const shot={ id, x:Math.round(sx), y:Math.round(sy), a:+angle.toFixed(3), spd, dmg,
      ref:!!ref, pierce:pierce||0,
      explosive:opts.explosive||0, ricochet:opts.ricochet||0,
      radius };
    outShots.push(shot); seenShots.add(id);
    bullets.push({ id, owner:myId, x:sx, y:sy,
      vx:Math.cos(angle)*spd, vy:Math.sin(angle)*spd,
      dmg, reflected:!!ref, pierce:pierce||0,
      explosive:opts.explosive||0, ricochet:opts.ricochet||0,
      radius, born:Date.now() });
  }
  ...
  function fire(){
    ...
    const explosive=me.mods.explosive||0;
    const ricochet=me.mods.ricochet||0;
    const spread=me.mods.spreadShot||0; // shotgun extra pellets
    const burst=me.mods.rapidBurst||0;  // extra burst bullets

    const opts={ explosive, ricochet };

    // Multishot (wide spread left/right)
    if(ms>0){
    ...
    // Shotgun spread
      ...
      for(let i=0;i<pellets;i++){
        const a=me.aim+(Math.random()-0.5)*0.5;
        spawnBullet(a, spd*(0.8+Math.random()*0.4), dmg*(0.6+Math.random()*0.4), false, pierce, opts);
      }
    }
    // Normal / burst
    else{
  ...
  function gainXp(n){
    ...
    while(me.xp>=need){ me.xp-=need; me.level++; need=xpForLevel(me.level);
      play('levelup',0.7);
      openCardDraft();
    }
  }
  ...
  function openCardDraft(){
    ...
    draftOpen=true;
    play('cardFan2',0.6);
    const picks=rollCards(3); // now 3 cards
    cardLayer.innerHTML='';
    ...
    picks.forEach((ab,i)=>{
      ...
      setTimeout(()=>{ card.style.transition='transform .28s cubic-bezier(.34,1.56,.64,1)';
  ...
  function selectCard(ab, card){
    ...
    setTimeout(()=>{
      ...
      const ti=setInterval(()=>{ const s=1-(f/flips.length);
  ...
  function buildLeaderboard(root){
    ...
    lbEl.id='caLeaderboard';
    lbEl.style.cssText=`
      position:absolute; top:16px; right:18px; z-index:20; pointer-events:none;
      width:200px; font-family:'JetBrains Mono',monospace; font-size:11px;
      background:rgba(18,18,22,.78); border:1px solid #2a2a32;
      border-radius:10px; padding:9px 11px; color:#ececef;
    `;
  ...
  function updateLeaderboard(){
    ...
    // Build list: me + others
    const all=[
      { name:me.name||'You', level:me.level, points:me.points, elims:me.elims, color:me.color, isMe:true }
    ];
    for(const id in others){
      const o=others[id];
      all.push({ name:o.name||'???', level:o.level||1, points:o.points||0, elims:o.elims||0, color:o.color||'#aaa', isMe:false });
    }
  ...
  function update(dt){
    ...
    for(let i=bullets.length-1;i>=0;i--){
      ...
      // world bounds
      let oob=false;
      if(b.x<-40||b.x>WORLD_W+40||b.y<-40||b.y>WORLD_H+40) oob=true;
      if(oob){ bullets.splice(i,1); continue; }

      // obstacle blocking / ricochet
      let blocked=false;
      for(const o of obstacles){
        if(b.x>=o.x&&b.x<=o.x+o.w&&b.y>=o.y&&b.y<=o.y+o.h){
          if((b.ricochet||0)>0){
            // simple bounce: flip whichever axis caused entry
            const cx=o.x+o.w/2, cy=o.y+o.h/2;
            const ox=Math.abs(b.x-cx)/(o.w/2), oy=Math.abs(b.y-cy)/(o.h/2);
      ...
      if(b.owner===myId){
        ...
        for(const id in others){ const o=others[id]; if(!o.alive) continue;
          if(d2(b.x,b.y,o.x,o.y)<(PLAYER_R+brad)**2){
            outDmg.push({ id:uid(), target:id, amount:Math.round(b.dmg) });
            gainUlt(1); gainXp(Math.round(b.dmg)*XP_PER_DMG);
            if(me.mods.lifesteal>0) me.hp=Math.min(MAX_HP,me.hp+me.mods.lifesteal);
            spawnParticles(b.x,b.y,'#ffb13b',5,120,0.3);
            play('hitEnemy',0.4);
            // Explosive on player hit too
  ...
  function draw(){
    ...
    for(const o of obstacles){
      ...
      if(o.type==='small'){
        // crate style
        ctx.fillStyle='#1e1a14'; ctx.fillRect(sx,sy,o.w,o.h);
        ctx.strokeStyle='rgba(160,120,60,0.6)'; ctx.lineWidth=1.5; ctx.strokeRect(sx,sy,o.w,o.h);
        ctx.strokeStyle='rgba(160,120,60,0.25)'; ctx.lineWidth=1;
        ctx.beginPath(); ctx.moveTo(sx,sy); ctx.lineTo(sx+o.w,sy+o.h); ctx.stroke();
        ctx.beginPath(); ctx.moveTo(sx+o.w,sy); ctx.lineTo(sx,sy+o.h); ctx.stroke();
      } else {
    ...
    for(const b of bullets){ const sx=b.x-camera.x, sy=b.y-camera.y;
      if(sx<-20||sx>VIEW_W+20||sy<-20||sy>VIEW_H+20) continue;
      const brad=b.radius||BULLET_R;
      ctx.beginPath(); ctx.arc(sx,sy,b.reflected?brad+2:brad,0,Math.PI*2);
      let bcolor;
      if(b.reflected) bcolor='#fff';
      else if(b.explosive>0) bcolor='#ff8c42';
      else if(b.owner===myId) bcolor='#fff';
      else bcolor='#ff8b8b';
      ctx.fillStyle=bcolor;
      const glowColor=b.explosive>0?'#ff8c42':b.reflected?'#c77dff':'#ff3b5c';
      ctx.shadowColor=glowColor; ctx.shadowBlur=b.explosive>0?18:10; ctx.fill(); ctx.shadowBlur=0;
    }
  ...
  function spriteKey(p){
    ...
    // walkShoot special case
    if(anim==='walk' && p.anim==='shoot') return ch+'_walkShoot'+fr;
    return ch+'_'+anim+fr;
  }
  function getSprite(p){
    // Try walkShoot if moving+shooting
    const ch=p.char||'pumpkin';
    const anim=p.anim||'idle';
    const fr=(p.frame?1:0)+1;
  ...
  function drawPlayer(p,isMe){
    ...
    if(img){
      ...
      ctx.fillStyle=pcolor; ctx.shadowColor=pcolor; ctx.shadowBlur=isMe?14:6; ctx.fill(); ctx.shadowBlur=0;
  ...
  function applyCharBonus(){
    // called once on start and after respawn
    // (rich dmg bonus handled inline in fire(), zaid speed in effSpeed())
  }
  ...
  function doStart(){
    ...
    me.id=myId; me.name=n; me.color=myColor; me.char=selectedChar;
  ...
  function buildCharacterPicker(gateCard){
    // Insert character picker into gate card before the button
    const pickerDiv=document.createElement('div');
    pickerDiv.id='caCharPicker';
    pickerDiv.style.cssText='display:flex;gap:10px;margin-bottom:14px;justify-content:center;';
    for(const cid in CHARACTERS){
      const ch=CHARACTERS[cid];
      const btn=document.createElement('button');
      btn.className='ca-char-btn'+(cid==='pumpkin'?' active':'');
      btn.dataset.char=cid;
      btn.style.cssText=`
        flex:1; background:${cid===selectedChar?'rgba(199,125,255,0.15)':'#0f0f12'};
        border:2px solid ${cid===selectedChar?ch.color:'#2a2a32'};
        color:#ececef; padding:10px 6px; border-radius:9px; cursor:pointer;
      ...
      btn.addEventListener('click',()=>{
        ...
        pickerDiv.querySelectorAll('.ca-char-btn').forEach(b=>{
          const bch=CHARACTERS[b.dataset.char];
          const active=b.dataset.char===cid;
          b.style.background=active?'rgba(199,125,255,0.15)':'#0f0f12';
          b.style.border=`2px solid ${active?bch.color:'#2a2a32'}`;
        });
      });
      pickerDiv.appendChild(btn);
    }
    const btnEl=gateCard.querySelector('.ca-btn');
    gateCard.insertBefore(pickerDiv, btnEl);
  }
  ...
  function cacheDom(root){
    ...
    // inject character picker into gate
    const gateCard=root.querySelector('.ca-gate-card');
    if(gateCard) buildCharacterPicker(gateCard);
  }
  ...
  ClaudeArena.init=function(opts){
    ...
    // Try to restore last character choice
    try{ const sc=localStorage.getItem('caChar'); if(sc&&CHARACTERS[sc]) selectedChar=sc; }catch(e){}

    // SFX mapping — using Balatro sfx names that fit the actions
    // shoot → button, dash → whoosh, hit (self) → glass1, hitEnemy → foil2
    // death → explosion1, levelup → coin5, shield → foil1
    // We add aliases to make code readable
    const sfxMap={
      shoot:'button', dash:'whoosh', hit:'glass1', hitEnemy:'foil2',
      death:'explosion1', levelup:'coin5', shield:'foil1',
    };
    ['button','whoosh','foil1','foil2','glass1','explosion1','coin3','coin5',
     'cardFan2','cardSlide1','cardSlide2','highlight1','highlight2','paper1']
